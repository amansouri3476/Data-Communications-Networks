...
//  Enable AODV
  ...
  uint16_t sinkPort = 6;
  Address sinkAddress (InetSocketAddress (ifcont.GetAddress (9), sinkPort)); // interface of n3
  PacketSinkHelper packetSinkHelper ("ns3::UdpSocketFactory", InetSocketAddress (Ipv4Address::GetAny (), sinkPort));
  ApplicationContainer sinkApps = packetSinkHelper.Install (c.Get (9)); //n3 as sink
...
// Set Mobility for all nodes
  ...
  positionAlloc ->Add(Vector(100, 0, 0)); // node1
  positionAlloc ->Add(Vector(100, -100, 0)); // node2
  positionAlloc ->Add(Vector(100, -200, 0)); // node3
  positionAlloc ->Add(Vector(400, -300, 0)); // node4
  positionAlloc ->Add(Vector(300, -300, 0)); // node5
  positionAlloc ->Add(Vector(300, -200, 0)); // node6
  positionAlloc ->Add(Vector(300, -100, 0)); // node7
  positionAlloc ->Add(Vector(200, -200, 0)); // node8
  positionAlloc ->Add(Vector(400, -150, 0)); // node9
  positionAlloc ->Add(Vector(400, -400, 0)); // node10
  ...
  AnimationInterface::SetConstantPosition (c.Get (0), 100, 0);
  AnimationInterface::SetConstantPosition (c.Get (1), 100, -100);
  AnimationInterface::SetConstantPosition (c.Get (2), 100, -200);
  AnimationInterface::SetConstantPosition (c.Get (3), 400, -300);
	AnimationInterface::SetConstantPosition (c.Get (4), 300, -300);
	AnimationInterface::SetConstantPosition (c.Get (5), 300, -200);
	AnimationInterface::SetConstantPosition (c.Get (6), 300, -100);
	AnimationInterface::SetConstantPosition (c.Get (7), 200, -200);
	AnimationInterface::SetConstantPosition (c.Get (8), 400, -150);
	AnimationInterface::SetConstantPosition (c.Get (9), 400, -400);
  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
    {
      ...
      if ((t.sourceAddress=="10.1.2.1" && t.destinationAddress == "10.1.2.9"))
      {
